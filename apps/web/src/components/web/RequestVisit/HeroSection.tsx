...
type PremiumProduct = {
  id: string;
  verificationType: "physical" | "virtual";
  price: string;
  currency: string;
  description: string;
...
type Step2FormProps = {
  ...
  selectedPlan: PremiumProduct | undefined;
...
function Step2Form({
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    // 2. Create Payment Intent using your backend API
    ...
    try {
      ...
      const paymentIntentResponse = await fetch(
        `${baseUrl}/api/${apiVersion}/public/billing/create-payment-intent`,
        {
          method: "POST",
      ...
      clientSecret = paymentIntentData.clientSecret; // Get clientSecret for confirmation
    } catch (err) {
      if (err instanceof Error) {
        setError(err.message || "Payment intent creation failed");
      } else {
    ...
    try {
      ...
      console.log("Confirmed payment intent ID:", paymentIntent.id);
    } catch (err) {
    ...
    // 4. Prepare payload for premium profile request (now with confirmed payment)
    const payload = {
      ...
      paymentIntentId: paymentIntentId, // This will now be a confirmed payment intent
      agree: accountForm.agree,
    };

    console.log("Sending verification request with payload:", payload);

    // 5. Call API for premium profile request
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || "";
      const apiVersion = process.env.NEXT_PUBLIC_API_VERSION || "v1";
      const response = await fetch(
        `${baseUrl}/api/${apiVersion}/public/residence-verification`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
function HeroSection() {
  ...
  // Fetch residence verification pricing
  const fetchPremiumProducts = async () => {
    ...
    try {
      ...
      const apiVersion = process.env.NEXT_PUBLIC_API_VERSION || "v1";
      const url = `${baseUrl}/api/${apiVersion}/public/residence-verification/pricing`;
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("Residence verification pricing:", data);
        setPremiumProducts(data.data || data || []);
      } else {
        console.error(
          "Failed to fetch residence verification pricing:",
          response.status
        );
        setPremiumProducts([]);
      }
    } catch (error) {
      console.error("Error fetching residence verification pricing:", error);
      setPremiumProducts([]);
    } finally {
    ...
    <Elements stripe={stripePromise}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
        ...
        <h1 className="text-3xl md:text-5xl font-semibold text-white max-w-4xl leading-tight">
          Request a Visit to Elevate Your Property&apos;s Rankings
        </h1>
        <p className="text-white text-lg max-w-2xl mt-4">
          Whether it&apos;s a virtual tour or a physical visit, showcase your
          property to our team of experts and enhance your visibility in the
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl flex w-full max-w-3xl min-h-[500px] overflow-hidden">
            ...
            <div className="bg-[#F7F6F2] p-8 w-1/3 flex flex-col justify-between">
              ...
              <div>
                <h2 className="text-2xl font-semibold mb-2">Request a Visit</h2>
                <p className="text-gray-600 text-base">
                  Struggling to choose the right plan? Let us help you cut
                  through the confusion and find the plan that fits your
                  residence and drives results.
                </p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl flex w-full max-w-3xl min-h-[500px] overflow-hidden">
            ...
            <div className="bg-white p-10 w-2/3 flex flex-col justify-center">
              <div className="mb-6">
                <h3 className="text-2xl font-semibold mb-6 text-gray-900">
                  Thank you! Your premium profile request has been submitted
                  successfully.
                </h3>
                <p className="text-gray-600 text-base mb-6">
                  We have received your request for a premium residence profile.
